...
public class Spel {
    ...
    public boolean LoginPlayer(String gebruikersnaam, char[] wachtwoord) throws SQLException, NoSuchAlgorithmException, UnsupportedEncodingException{
        MessageDigest md = MessageDigest.getInstance("SHA-256");
        md.update(new String(wachtwoord).getBytes("UTF-8"));
        byte[] digest = md.digest();
        String test = "";
        ...
        int login = myStmt.executeUpdate("select wachtwoord from speler where gebruikersnaam = " + gebruikersnaam);
        
        if (login == wachtwoord.hashCode()){
            return true;
        } else {
            return false;
        }
